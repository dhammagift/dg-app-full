...
(function () {
    ...
    function report$write() {
        ...
        if (!plugin || typeof plugin.write !== 'function') {
            ...
            console.log('DG_SELFTEST_REPORT ' + JSON.stringify(report));
            return Promise.resolve(false);
        }
        return Promise.resolve(plugin.write({ json: JSON.stringify(report, null, 2) }))
            .then(function () { return true; })
            .catch(function (e) { console.error('dg-selftest: write failed: ' + e.message); return false; });
    }

    // Leave the app on a real reader page, so the screenshots CI takes afterwards show the app
    // rather than the home screen. ?_nativeRoute= is the same handoff the Android shortcuts use:
    // a pushState'd path cannot be loaded directly on a device (the asset server has no file behind
    // it), so the route rides on the root URL and the page rewrites it before its own bootstrap.
    function showReader() {
        var target = '/?_nativeRoute=' + encodeURIComponent('/dn22:2.2');
    ...
    waitForLibrary()
        ...
        .then(function (written) {
            ...
            // on a reader page is what makes the screenshots CI takes show the app itself.
            showReader();
        });
})();
