 ...
 * browser tab and no share chooser in between. Kept from the Trusted Web Activity this app
 * replaces — losing it would have been a silent regression.
 *
 * It carries no logic of its own. The tap starts MainActivity with the same "route" extra a
 * launcher shortcut uses (res/xml/shortcuts.xml), and MainActivity turns that into the URL — so
 * the tile, a shared link and a tapped shortcut all arrive through one path.
 *
 * The tile is invisible until the reader adds it: pull the shade down, edit the tiles, drag
 * "Dict.Dhamma.Gift" into the panel. That is Android's rule for every app, not something an app can
 * skip.
 */
