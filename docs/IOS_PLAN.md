...
4. **Динамические шорткаты сделаны** (2026-09-17): плагин `DgShortcuts` берёт список «недавно
   читал» от страницы (тот же контракт `set({items})`, что у Android) и кладёт его в
   `UIApplication.shared.shortcutItems`. Тап по любому шорткату — и динамическому, и четырём
   статическим из Info.plist — открывает маршрут через **ту же** ссылку `dhammagift://route/…`,
   которую приложение уже принимает и проверяет; заодно это починило статические шорткаты, которые на
   iOS до сих пор не делали ничего. Проверки в симуляторе: round-trip `set`→`get` (что система
   реально хранит) и тап (`open('/4as')` → приложение оказалось на `/4as`, видно в `deepLinksSeen`).
...
Платный аккаунт оформляет владелец — пошаговый список на день аккаунта (что создать, какие три
секрета положить в GitHub, что заменить в AASA, какие четыре проверки сделать на телефоне) лежит в
`docs/IOS_ACCOUNT_DAY.md`. Релизная джоба `ios-release` уже написана: она архивирует, подписывает с
`-allowProvisioningUpdates`, грузит в TestFlight и кладёт `.ipa` в артефакты — и до появления секретов
просто пропускает шаги, ничего не ломая.
...
схема, шара и возврат логина работают. Конфигурация Release теперь подписана `Apple Distribution`
(иначе archive просит development-профиль, а он требует зарегистрированных устройств), поэтому
сборке на личную команду нужен ещё и `CODE_SIGN_IDENTITY="Apple Development"` — вместе с той же
подменой entitlements.
