...
// Android System WebView has no window.speechSynthesis, so the reader's voice player
// (read/js/voice.js, the site's own) had nothing to speak with and threw on its first call: no
// sound, and a close button that never closed. This stands in for the part of Web Speech that
// player uses (speak/cancel/getVoices/voiceschanged; utterance text/lang/rate/voice/onend/onerror)
// over Android's TextToSpeech (DgTtsPlugin). A browser that has the real API keeps it.
(function () {
